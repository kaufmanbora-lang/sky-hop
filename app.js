(function () {
  ...
  var SESSION_KEY = "sky-hop-session-v1";
  ...
  function init() {
    ...
    configureInputMode();
    bindEvents();
    setAuthMode(profile && profile.account ? "login" : "register");
    if (profile && profile.account && localStorage.getItem(SESSION_KEY) === "active") {
      showMenu();
    } else {
      navigateTo("auth-screen", ".auth-tabs .active");
    }
  }
  ...
  function bindEvents() {
    document.addEventListener("click", function (event) {
      ...
      if (event.target === dom.canvas && game.status === "running") applyUpCommand();
    });
    ...
    dom.email.addEventListener("input", clearAuthError);
    dom.password.addEventListener("input", clearAuthError);
  ...
  function handleAction(action, target) {
    audio.activate();
    if (action === "auth-tab") setAuthMode(target.dataset.mode);
  ...
  function setAuthMode(mode) {
    authMode = mode;
    var tabs = document.querySelectorAll("[data-action='auth-tab']");
    for (var i = 0; i < tabs.length; i += 1) {
      var active = tabs[i].dataset.mode === mode;
      tabs[i].classList.toggle("active", active);
      tabs[i].setAttribute("aria-selected", active ? "true" : "false");
    }
    dom.authSubmit.textContent = mode === "register" ? "СОЗДАТЬ АККАУНТ" : "ВОЙТИ";
    dom.password.autocomplete = mode === "register" ? "new-password" : "current-password";
    clearAuthError();
  }

  async function submitAuth() {
    var email = dom.email.value.trim().toLowerCase();
    var password = dom.password.value;
    if (!/^\S+@\S+\.\S+$/.test(email)) return setAuthError("Введите корректную почту");
    if (password.length < 6) return setAuthError("Пароль должен содержать минимум 6 символов");
    dom.authSubmit.disabled = true;
    dom.authSubmit.textContent = "ПОДОЖДИТЕ…";
    try {
      if (authMode === "register") await registerAccount(email, password);
      ...
      dom.authSubmit.textContent = authMode === "register" ? "СОЗДАТЬ АККАУНТ" : "ВОЙТИ";
    }
  }

  async function registerAccount(email, password) {
    if (profile && profile.account) throw new Error("На этих очках уже зарегистрирован аккаунт");
    var deviceId = getDeviceId();
    var salt = randomHex(16);
    var hash = await hashPassword(password, salt);
    profile = defaultProfile();
    profile.account = { email: email, salt: salt, hash: hash, deviceId: deviceId, createdAt: new Date().toISOString() };
    saveProfile();
  }

  async function loginAccount(email, password) {
    if (!profile || !profile.account) throw new Error("Сначала создайте аккаунт на этих очках");
    if (profile.account.deviceId !== getDeviceId()) throw new Error("Аккаунт привязан к другим очкам");
    if (profile.account.email !== email) throw new Error("Почта не совпадает");
    var hash = await hashPassword(password, profile.account.salt);
    if (hash !== profile.account.hash) throw new Error("Неверный пароль");
  }
  ...
  async function hashPassword(password, salt) {
    ...
    throw new Error("Для безопасного входа откройте игру по HTTPS");
  }
  ...
  function setAuthError(message) { dom.authError.textContent = message; }
  ...
  function renderMenu() {
    ...
    document.getElementById("menu-email").textContent = profile.account.email;
