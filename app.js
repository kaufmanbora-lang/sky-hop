(function () {
  ...
  var FLAP_VELOCITY = -320;
  var FLAP_CHAIN_BOOST = 34;
  var MAX_RISE_VELOCITY = -370;
  var MODIFIER_BIRD_COLLISION_RADIUS = 20;
  var MODIFIER_PIPE_CLEARANCE = 30;
  var IMAGE_LOAD_TIMEOUT_MS = 2600;
  var AUDIO_LOAD_TIMEOUT_MS = 3200;
  ...
  var audio = {
    ...
    loadAssets: function () {
      ...
      if (self.assetsAttempted) return Promise.resolve();
      self.assetsAttempted = true;
      var files = {
      ...
      self.assetsPromise = Promise.all(Object.keys(files).map(function (name) {
        ...
        var request = fetch(files[name])
          .then(function (response) {
            if (!response.ok) throw new Error("Audio asset failed: " + files[name]);
            return response.arrayBuffer();
          })
          .then(function (bytes) { return decodeAudioSafely(ctx, bytes); })
          .then(function (buffer) {
            if (buffer) self.buffers[name] = buffer;
          })
          .catch(function () {});
        return settleAfter(request, AUDIO_LOAD_TIMEOUT_MS);
      })).then(function () {
    ...
    startMusic: function () {
      ...
      if (!self.buffers.soundtrack) {
        if (!self.assetsAttempted) self.loadAssets().then(function () { self.startMusic(); });
        return;
      }
      var source = ctx.createBufferSource();
      var gain = ctx.createGain();
  ...
  function init() {
    ...
    bindEvents();
    showMenu();
    loadGameAssets().catch(function () {});
  }

  function settleAfter(promise, timeoutMs) {
  ...
  function decodeAudioSafely(context, bytes) {
    return new Promise(function (resolve) {
      var settled = false;
      var timer = window.setTimeout(function () { finish(null); }, AUDIO_LOAD_TIMEOUT_MS);
      ...
      try {
        var result = context.decodeAudioData(bytes.slice(0), finish, function () { finish(null); });
  ...
  function loadGameAssets() {
    ...
    assetsPromise = Promise.all(names.map(function (name) {
      return new Promise(function (resolve) {
        var image = new Image();
        var finished = false;
        var timer = window.setTimeout(done, IMAGE_LOAD_TIMEOUT_MS);
        function done() {
          if (finished) return;
          finished = true;
          window.clearTimeout(timer);
  ...
  function makeFallbackCanvas(width, height, painter) {
    var canvas = document.createElement("canvas");
    canvas.width = width;
    canvas.height = height;
    var context = canvas.getContext("2d");
    if (context) painter(context, width, height);
    return canvas;
  }

  function ensureFallbackAssets() {
    if (!assets.bird) {
      assets.bird = makeFallbackCanvas(92, 64, function (ctx) {
        ctx.fillStyle = "#ffd814";
        ctx.beginPath();
        ctx.ellipse(43, 34, 30, 23, -0.08, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#1fbfff";
        ctx.beginPath();
        ctx.ellipse(35, 42, 18, 11, -0.35, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#ff9b10";
        ctx.beginPath();
        ctx.moveTo(67, 30); ctx.lineTo(91, 37); ctx.lineTo(67, 43); ctx.closePath(); ctx.fill();
        ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.arc(55, 24, 8, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#031934"; ctx.beginPath(); ctx.arc(58, 24, 3.5, 0, Math.PI * 2); ctx.fill();
      });
    }
    if (!assets.pipe) {
      assets.pipe = makeFallbackCanvas(86, 320, function (ctx, width, height) {
        var gradient = ctx.createLinearGradient(0, 0, width, 0);
        gradient.addColorStop(0, "#238d22"); gradient.addColorStop(0.5, "#7ee72a"); gradient.addColorStop(1, "#12651b");
        ctx.fillStyle = gradient; ctx.fillRect(9, 0, width - 18, height);
        ctx.fillStyle = "#4fc31e"; ctx.fillRect(0, 0, width, 34);
        ctx.strokeStyle = "#0b4818"; ctx.lineWidth = 5; ctx.strokeRect(2.5, 2.5, width - 5, height - 5);
      });
    }
    if (!assets.reverse) assets.reverse = makeFallbackCanvas(48, 48, function (ctx) {
      ctx.fillStyle = "#955eff"; ctx.beginPath(); ctx.arc(24, 24, 22, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#ffffff"; ctx.font = "900 28px Arial"; ctx.textAlign = "center"; ctx.fillText("↕", 24, 34);
    });
    if (!assets.speed) assets.speed = makeFallbackCanvas(48, 48, function (ctx) {
      ctx.fillStyle = "#ffd814"; ctx.beginPath(); ctx.arc(24, 24, 22, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#17385e"; ctx.beginPath(); ctx.moveTo(27, 5); ctx.lineTo(13, 27); ctx.lineTo(24, 27); ctx.lineTo(19, 43); ctx.lineTo(37, 20); ctx.lineTo(26, 20); ctx.closePath(); ctx.fill();
    });
    if (!assets.star) assets.star = makeFallbackCanvas(32, 32, function (ctx) {
      ctx.fillStyle = "#ffd814"; ctx.beginPath(); ctx.arc(16, 16, 8, 0, Math.PI * 2); ctx.fill();
    });
    if (!assets.planet) assets.planet = makeFallbackCanvas(32, 32, function (ctx) {
      ctx.strokeStyle = "#50e6ff"; ctx.lineWidth = 4; ctx.beginPath(); ctx.ellipse(16, 17, 14, 5, -0.3, 0, Math.PI * 2); ctx.stroke();
      ctx.fillStyle = "#aa65ff"; ctx.beginPath(); ctx.arc(16, 16, 8, 0, Math.PI * 2); ctx.fill();
    });
    if (!assets.card) assets.card = makeFallbackCanvas(28, 36, function (ctx) {
      ctx.fillStyle = "#ffffff"; ctx.fillRect(2, 2, 24, 32); ctx.strokeStyle = "#ff3d57"; ctx.lineWidth = 3; ctx.strokeRect(2, 2, 24, 32);
    });
    if (!assets.impact) assets.impact = makeFallbackCanvas(72, 72, function (ctx) {
      ctx.fillStyle = "#ff4d5d"; ctx.beginPath(); ctx.arc(36, 36, 18, 0, Math.PI * 2); ctx.fill();
    });
  }

  function buildFallbackCollisionMasks() {
    collisionMasks.birdPoints = [];
    for (var y = -17; y <= 17; y += 3) {
      for (var x = -27; x <= 27; x += 3) {
        if ((x * x) / (27 * 27) + (y * y) / (17 * 17) <= 1) collisionMasks.birdPoints.push({ x: x, y: y });
      }
    }
    if (assets.pipe) {
      var width = assets.pipe.width || PIPE_WIDTH;
      var height = assets.pipe.height || 320;
      var alpha = new Uint8Array(width * height);
      alpha.fill(255);
      collisionMasks.pipe = { alpha: alpha, width: width, height: height };
    }
  }
  ...
  function startGame() {
    ...
    ensureFallbackAssets();
    if (!collisionMasks.birdPoints.length || !collisionMasks.pipe) {
      try {
        buildCollisionMasks();
      } catch (error) {
        buildFallbackCollisionMasks();
    ...
    launchRun();
    loadGameAssets().catch(function () {});
    audio.loadAssets().then(function () { audio.startMusic(); }).catch(function () {});
  }
  ...
  function spawnPipe(x) {
    ...
    if (game.score >= 7 && Math.random() < chance) {
      ...
      var modifierRadius = 21;
      var modifierInset = modifierInsetFromEdge(gap, modifierRadius);
      var gapTop = center - gap / 2;
      var gapBottom = center + gap / 2;
      modifier = {
        type: type,
        x: x + PIPE_WIDTH / 2,
        y: side < 0 ? gapTop + modifierInset : gapBottom - modifierInset,
  ...
  function modifierInsetFromEdge(gap, modifierRadius) {
    var mode = activeDifficulty();
    var collisionRadius = modifierRadius + MODIFIER_BIRD_COLLISION_RADIUS;
    var inset = gap - MODIFIER_PIPE_CLEARANCE - collisionRadius - mode.modifierSafeLane;
    return Math.max(modifierRadius + 4, inset);
  }
  ...
  function flap() {
    game.bird.vy = game.bird.vy <= FLAP_VELOCITY
      ? Math.max(MAX_RISE_VELOCITY, game.bird.vy - FLAP_CHAIN_BOOST)
      : FLAP_VELOCITY;
  ...
  function circleBirdCollision(modifier) {
    ...
    return dx * dx + dy * dy < (modifier.radius + MODIFIER_BIRD_COLLISION_RADIUS) * (modifier.radius + MODIFIER_BIRD_COLLISION_RADIUS);
  }
